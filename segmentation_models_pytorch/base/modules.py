...
class HybridAttentionModule(nn.Module):
    def __init__(self, in_channels, reduction=16):
        ...
        self.spatial_attention = nn.Sequential(
            nn.Conv2d(in_channels, in_channels // reduction, kernel_size=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_channels // reduction, 1, kernel_size=3, padding=1),
            nn.Sigmoid(),
        )
    ...
    def forward(self, x):
        ...
        channel_refined = x * channel_weight

        spatial_weight = self.spatial_attention(x)
        spatial_refined = x * spatial_weight


        fused = self.alpha * channel_refined + self.beta * spatial_refined
        return fused
class Attention(nn.Module):
    ...
    def __init__(self, name, **params):
        ...
        if name is None:
            ...
            self.attention = HybridAttentionModule(**params)
        else:
